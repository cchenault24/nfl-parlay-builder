...
export const fetchGamesByWeek = async (
  week?: number,
  year?: number
): Promise<NFLGame[]> => {
  try {
    ...
    throw new Error(
      `Failed to fetch NFL games from ESPN API for week ${week || 'current'}`
    )
  }
}
...
export const getCurrentNFLWeek = async (): Promise<number> => {
  try {
    ...
    if (data.week && data.week.number) {
      ...
      const adjustedWeek = await getAdjustedWeekBasedOnCompletion(
        apiWeek,
        data.events || []
      )
...
export const fetchTeamRoster = async (teamId: string): Promise<NFLPlayer[]> => {
  try {
    const response = await axios.get(
      `${API_CONFIG.ESPN_BASE_URL}/teams/${teamId}/roster`
    )
    ...
    const players: NFLPlayer[] = allAthletes.map(athlete => {
      return {
        ...
        position:
          athlete.position?.abbreviation || athlete.position?.name || 'Unknown',
...
export const fetchGameRosters = async (
  game: NFLGame
...
const getAdjustedWeekBasedOnCompletion = async (
  currentWeek: number,
  currentWeekEvents: any[]
): Promise<number> => {
  try {
