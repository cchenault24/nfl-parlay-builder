...
export const generateParlay = async (game: NFLGame): Promise<GeneratedParlay> => {
  try {
    ...
    const userPrompt = createParlayPrompt(game, homeRoster, awayRoster, varietyFactors);
    ...
    const result = parseAIResponse(response, game, homeRoster, awayRoster, varietyFactors);
